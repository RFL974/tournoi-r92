...
**#220**, `verifier` **`success`** — « 30 fichiers JavaScript vérifiés, aucun cassé » — et `deploy`
...
| **L2** | Textes de l'application — titres, métadonnées, signatures, valeurs de repli, `.ics`, commentaires devenus faux | ✅ **FAIT** — commit **`5bff881`**. ⚡ **50 points, et non 45** : le recomptage a trouvé **3 commentaires** que la première regex manquait, et ⭐ **l'`UID` du fichier calendrier**, qui portait `@generation-r92` — invisible d'une recherche sur « Génération R92 ». ⚠️ **Dette temporaire assumée** : le filtre reste `MOT_CLE_CLUB = 'racing'` **jusqu'à L8**. ⚡ **CORRECTION apportée par L8, et il faut la lire** : L2 avait écrit que la page Perfs était *« textuellement générique »* — **c'était inexact**. Son **titre** et son `<h1>` l'étaient bien *(`perfs.html`)*, mais `perfs.js:135` **injectait la valeur du filtre dans le DOM** : le message d'absence affichait littéralement *« Aucune équipe « racing » »*. ⛔ **Et il ne s'affichait QUE chez un organisateur qui n'est pas le Racing.** La neutralisation de L2 était donc réelle sur le HTML, incomplète sur le JS — trouvée par les agents A et B, indépendamment, pendant l'audit de L8 |
...
> ✅ **TRAITÉ le 2026-08-22.** Ce tableau est conservé **tel qu'il a été établi**, comme trace du
> périmètre décidé — ses `fichier:ligne` sont donc ceux d'**avant** L8 et ne correspondent plus.
> ⚠️ **Deux lignes ont été volontairement écartées** après analyse : le préfixe de bilan `R92 — ` et
> le menu du classeur, tous deux **invisibles de l'utilisateur** et rattachés à la réserve du nom
> technique du projet *(**D-041**, arbitrages A-2 et A-3)*.
...
##### PUB-2 — Accès autonome à la page publique  🏁 **TERMINÉ ET CLÔTURÉ** *(2026-08-26)* — ✅ **R-098 est CLOS depuis la clôture de PUB-4** *(D-055)*

> ⚡ **Ce titre annonçait « ⛔ R-098 reste OUVERT *(condition 5 reportée à PUB-4)* » : vrai du 2026-08-26 *(suite 2)* jusqu'à la clôture de PUB-4 le même jour *(suite 5)*.** ⭐ **La condition 5 a bien été honorée en PUB-4**, comme **D-052** l'avait organisé — ⚠️ **avec une réserve écrite** *(`RISQUES.md`, encadré « LA CONDITION 5 »)*.
...
| 🔴 **R-098 — l'anomalie trouvée en essayant, et son correctif** | ⚡ **La validation réelle a échoué à son PREMIER contrôle le 2026-08-24** : la carte était **inatteignable** sur un classeur non préparé *(verrou de navigation **antérieur** à PUB-2, que PUB-2 n'avait pas analysé)*. ✅ **Correctif FUSIONNÉ et PUBLIÉ le même jour** — fast-forward **`2ef9ce0` → `b8ce265`** *(**`9bdeb06`** le correctif · **`b8ce265`** la trace post-geste)*, run Pages **#221** `success`. Le garde-fou passe de l'**ÉCRAN** au **BOUTON**, sur **les trois parcours** *(barre latérale, assistant mobile, Vue classique)*. ✅ **R-098 est CLOS depuis la clôture de PUB-4** *(2026-08-26 suite 5, **D-055**)* — ⚠️ **avec la réserve écrite de sa condition 5**. ⚡ **CORRIGÉ APRÈS LE GESTE le 2026-08-26** : cette case a annoncé successivement *« 1 de ses 5 conditions de fermeture est remplie »*, puis *« ⛔ **R-098 reste OUVERT** — il y en a désormais 4 sur 5 »*. **Chacune était vraie à sa date**, et les **cinq** sont désormais honorées. Détail : `RISQUES.md` **R-098** et `SESSIONS.md` **§22** |
...
##### PUB-4 — Exécution du découplage  🏁 **TERMINÉ ET CLÔTURÉ** *(2026-08-26)*
...
**🏁 État CONSTATÉ de PUB-4** *(2026-08-26 — décision de clôture de Romain, **D-055**)*

| | |
|---|---|
| ✅ **Obligation ① — le découplage** | **PROUVÉE EN RÉEL.** Le tournoi fictif a été **publié puis masqué** pendant qu'on observait les deux sites : ⭐ **la page publique Maxilou a réagi**, ⛔ **la vitrine n'a pas bougé d'un octet**. Cycle refermé : `non → publié → non` |
| ✅ **Obligation ② — les 3 preuves héritées de PUB-2** | **R-098 condition 5** *(« Masquer » jamais grisé)* · **contrôle §21.10 ter 6** *(Publier → vert, adresse identique)* · **contrôle §21.10 ter 8** *(Masquer → même adresse, retour à « à venir »)*. ⭐ **Relevées séparément**, comme le plan l'exigeait |
| ⚠️ **La réserve de la condition 5** | Elle a été honorée **par observation** *(4 affichages, dont un téléphone réel)* **et par lecture du code** *(publié ⇒ la branche retourne avant toute lecture des prérequis)*. ⛔ **Le scénario littéral « publié + prérequis volontairement cassés » N'A PAS ÉTÉ JOUÉ, et ne doit jamais être présenté comme joué** — `RISQUES.md`, encadré « LA CONDITION 5 » |
...
> | **⑤ Frontend publié** *(GitHub Pages)* | ✅ **OUI, ET OBSERVÉ** — run **32712062024** sur **`8dfd28a`**, **`success`**, ses **deux jobs** compris, 2026-08-24 09:32 UTC ; ⭐ **puis la page servie a été ouverte à la main le 2026-08-24** et le **nouveau dialogue s'est affiché**. ⚡ **Cette case portait la réserve « non observé »** — levée. ⭐ **La fenêtre backend-nouveau / frontend-ancien est FERMÉE** |
...
> | **⑦ Vérifiée en réel** | ✅ **OUI — 2026-08-24.** ⚡ **Cette case disait NON**, le geste destructif n'ayant pas été exercé. Il l'a été, **de bout en bout** : frontend publié → `CONTINUER` → `OUI, TOUT EFFACER` → backend v156 → classeur *(« Supprimés : 2 catégories, 38 équipes, 10 poules, 51 matchs »)*. ⭐ **Relevé DANS `Config`, valeur stockée, avant et après** : **10/10 conservés · 26/26 effacés · 2/2 récompenses effacées** · `tournoi_publie` = `non` · `tournoi_affiche_id` et `parking_photo_id` vidés · **les 26 clés existent toujours**. ⚠️ **`org_type_terrain` a été contrôlé sur la CELLULE**, pas sur l'affichage recalculé — le piège annoncé a été évité |
...
**④ ✅ CE QUI EST DÉSORMAIS EN SERVICE ET PROUVÉ EN RÉEL** *(2026-08-27)*.

| | |
|---|---|
| ✅ **Intégré et poussé** | Avance rapide **pure** *(`--ff-only`)*, ⛔ aucun commit de fusion. Repères exacts : [`ETAT.md`](ETAT.md) *(§8 quater)* |
| ✅ **Backend redéployé** | Version **158 → 159**, ⛔ **même URL, mêmes droits, aucun second déploiement**. ⚡ *(Cette case disait « ⛔ **Backend non redéployé** — le serveur porte toujours l'ancien `Code.gs` » : vrai jusqu'à cette date.)* |
