...
| **D-027** | **L'attente est annoncée, jamais subie.** Page publique : indicateur de chargement **animé** + courte explication du délai, **sans aucun chiffre** (un délai annoncé devient une promesse — or 4 % des appels dépassent 10 s). Page de saisie : animation en **deux temps**, « c'est pris en compte » → « c'est bon ». ⚠️ **4 réserves** : 60 s ne donnera pas 4× la capacité (le bouton « Rafraîchir » concentre la charge au pic → **30 s d'abord**) · une animation **ne doit jamais mentir** (3 issues : ça arrive / c'est arrivé / **ça a échoué**) · **CSS pur**, jamais d'image · **l'animation prime sur l'accélération**, car 60 % du temps d'une validation est incompressible | ✅ **Validée (session 10)** — **conception proposée par Romain** |
