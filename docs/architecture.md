...
| ⏳ ⚡ **Lot B2-2 — l'ENGAGEMENT des clubs n'est plus VIDÉ, il CHANGE D'ÉDITION.** L'ancienne participation reste, rattachée à l'édition **fermée** ; la nouvelle édition n'en a **aucune**. ⛔ Il n'y a **plus de liste de colonnes** à vider — donc plus rien à oublier *(cause de **R-099** / **R-100**)*. ⭐ Le **carnet**, lui, ne bouge pas | ⏳ 🆕 ⭐ **Le CARNET `Clubs`** — intégralement **conservé**, y compris les clubs marqués `actif = non` |
...
> | ✅ **R-106** *(`tournoi_id`)* | **CLOS le 2026-08-27** *(**D-058**)* — `edition_id` et l'onglet `Editions` sont **en service et prouvés en réel** : migration, 3 régénérations sans mouvement de l'identifiant, et **reset nominal réel** *(ancienne fermée, neuve ouverte, une seule active)*. ⭐ **`tournoi_id` reste, à dessein, reposé à chaque génération** : c'est son rôle *(**D-057**)*. ⛔ Le **cas d'ÉCHEC** du reset reste couvert par le **harnais**, pas par la production. ⚡ *(Cette case a annoncé « OUVERT — l'identifiant est bien effacé au reset, mais il reste reposé à CHAQUE génération », puis « une réponse existe DANS LE DÉPÔT — ⛔ ni déployée, ni migrée ». **Chacune était vraie à sa date.**)* |
